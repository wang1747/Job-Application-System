...
#### 可选扩展：模型设置与用户自带 API Key（BYOK）

目标：项目首次上线时不配置任何默认 API Key；用户必须先完成模型设置，才能使用 JD 解析、简历优化、面试准备等依赖 LLM 的功能。

功能要求：

- 首次注册或登录后，如果当前用户没有模型配置，进入主界面时强制跳转到“模型设置”页面。
- 新增“模型设置”页面，支持选择预设模型，例如 DeepSeek、OpenAI、Codex、Moonshot、通义千问、智谱 GLM 等 OpenAI 兼容模型。
- 每个模型预设自动带出默认 Base URL 和默认模型名；高级用户可自定义 Base URL 和模型名。
- 用户选择模型后填写对应 API Key。
- 保存前必须执行“测试连接”，测试成功后才允许保存。
- 保存成功后，前端只显示掩码，例如 `sk-****1234`。
- 提供“重新填写并覆盖”和“清除我的 Key”操作；清除后 LLM 功能不可用，直到用户重新配置，不回退到服务器默认 Key。
- `User` 表新增字段：`llm_provider`、`llm_base_url`、`llm_model`、`encrypted_api_key`。
- `llm.py` 改为根据当前用户动态创建 `ChatOpenAI`，不再使用全局缓存单例。
- 新增接口：保存模型配置、测试连接、获取掩码配置、清除模型配置。
- 所有依赖 LLM 的接口在用户未配置模型时返回明确的引导错误，例如“请先完成模型设置”。

模型预设示例：

| 模型 | 默认 Base URL | 默认模型名 |
| --- | --- | --- |
| DeepSeek | https://api.deepseek.com | deepseek-chat |
| OpenAI | https://api.openai.com/v1 | gpt-4o-mini |
| Codex | 待确认的 OpenAI 兼容地址 | 待确认模型名 |
...
- API Key 加密存储，加密方案使用 AES-GCM 或 Fernet，加密密钥从服务器 `.env` 读取。
- 加密密钥不能出现在代码、测试快照、README 或 Docker 镜像中。
- API Key 不写日志，不进入异常信息、错误响应、数据库备份。
- API Key 不返回给前端，只返回掩码。
- API Key 不进入简历文件、测试快照或 README。
- 用户提交的 Base URL 必须校验为 http/https，并校验不是内网地址，防止 SSRF 风险。
- 每个用户只能读写自己的 Key，路由层和服务层都必须校验 `user_id`。

失败与回退策略：

- 用户配置了 Key，就只使用该用户的 Key，不自动回退服务器 Key。
- 用户 Key 认证失败、额度不足或请求超时时，返回明确错误，并引导用户去“模型设置”更新。
- 服务器不提供默认 API Key；生产环境 `.env` 中不配置 `DEEPSEEK_API_KEY` 作为用户回退项。
- 开发环境如需本地调试，可以使用测试用的假 LLM 或临时 Key，不进入生产配置。

使用限制：

- 每个用户设置 Key 后，按用户维度做调用频率限制和超时控制。
- 防止用户使用自己的 Key 刷接口导致资源占用过高。
- 记录调用次数、成功/失败次数、耗时，但不记录 Key。

前端交互：

- 模型设置页展示当前模型、Base URL、模型名、掩码 Key。
- 提供模型预设下拉框、自定义配置开关、测试连接、保存、清除我的 Key。
- 保存成功、测试失败、清除成功都有明确反馈。
- 未配置模型时，所有依赖 LLM 的页面显示“请先完成模型设置”和跳转按钮。

验证标准：

- 用户 A 设置自己的 DeepSeek Key 后，不影响用户 B 的模型配置。
- 未配置 Key 的用户无法调用依赖 LLM 的接口，并收到明确引导。
- 页面刷新后 Key 仍只显示掩码。
- 清除 Key 后 LLM 功能不可用，直到用户重新配置。
- 测试连接失败时不会保存配置。
- 接口和日志中不出现完整 API Key。
- 用户 A 无法读取或修改用户 B 的模型配置。
- Base URL 为内网地址或非 http/https 时被拒绝。
...
- n8n 不作为核心架构，只在提醒和外部集成阶段引入。
- n8n 作为独立容器加入 `docker-compose.yml`，与 OfferFlow 前后端一起部署。
- OfferFlow 暴露 Webhook，n8n 负责定时触发和消息推送，不把核心业务逻辑搬进 n8n。
- 典型集成：邮件、企业微信、钉钉、通用 Webhook、招聘网站、表单、Excel、飞书表格。
- 职责边界：LangGraph 负责 AI Agent 工作流，n8n 负责外部自动化集成。
- 验证：n8n 容器启动成功，OfferFlow 推送待跟进投递或即将面试记录，n8n 能按规则发送提醒。
...
- 阶段一：完成。Docker 前后端部署、Nginx、健康检查、CORS、项目名均已落地。
- 阶段二：完成。注册登录、JWT、用户数据隔离、前端登录闭环、模型设置、BYOK、未配置引导、演示账号均已完成。
- 阶段三：完成。GitHub Actions CI 已配置，包含后端测试、前端 lint/build、Docker 构建。
- 阶段四：完成。简历导出 PDF/Word、OCR 截图导入、批量 JD 匹配、提醒推送、n8n 容器均已完成。
- n8n 已提供基础工作流模板：`n8n/workflows/offerflow-reminders.json`。
- 阶段五：README 已重写；项目截图和线上地址待部署后补充。

剩余外部操作：

- 在真实服务器执行 `docker compose up --build -d`。
- 部署后补充项目截图和线上访问地址。
- 在 n8n UI 中导入 `n8n/workflows/offerflow-reminders.json` 并按实际渠道继续配置，或直接配置 `REMINDER_WEBHOOK_URL`。
...
可改进点（按影响排序）
README 还是 Vite 默认模板（"React + TypeScript + Vite"那套），没有项目介绍、启动方式、VITE_API_URL 配置说明。这是最该补的一块——面试官或合作者第一眼看的就是它。
index.html 写的是 <html lang="en">，但全站中文 → 应改成 lang="zh-CN"（SEO/可访问性基础项）。
initialize() 被重复调用：App、PrivateRoute、AdminRoute 三个组件的 useEffect 各自调一次 initialize()，首屏会并发 3 个 /auth/me。应在 App 层初始化一次，守卫里只读取 store 状态，避免竞态和浪费请求。
登录态健壮性有缺口：authStore 里只有 me() 失败才清 token；其他接口返回 401 时不会自动登出，用户会带着"死 token"留在界面。建议加一个响应拦截：所有 401 统一清 token + 跳登录。另外 token 永久不过期、无刷新机制（个人工具可接受，但面试时会被追问，最好能说出取舍）。
Dashboard 无模型配置就强制跳 /settings：每次进总览都被弹去设置页，体验上"永远看不到仪表盘"。建议改成非阻断的提示条（banner），而不是重定向。
类型重复：JDAnalysis.tsx 里手写了 ParsedResult，和 types/index.ts 的 JDParseResult 重复，容易日后不一致——直接复用共享类型。
导航 active 状态有点脆弱：Layout 用一长串 startsWith if/else 推导 active，且把 onNavigate 传成空函数（() => {}）。可以改用 NavLink 的 end + useLocation，更健壮也更简洁（顺手清掉那段死代码）。
静默吞错：Dashboard.loadAll 是空 catch {}，其它页多是 console.error + toast。可以引入一个全局 toast（如 react-hot-toast）和 loading 骨架屏，体验更专业。
缺 .env.example 与测试：VITE_API_URL 没人文档化；API client 是纯函数式、很好测，建议至少补几个单测（也是面试加分项）。